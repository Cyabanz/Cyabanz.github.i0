...
fetch('data.json')
    ...
    .then(data => {
        ...
        let startX = 0;
        let endX = 0;
        let isDragging = false; // Track whether a swipe is in progress
        ...
        carouselInner.addEventListener('pointerdown', (event) => {
            ...
            isDragging = true; // Indicate dragging has started
            event.preventDefault(); // Prevent default scrolling behavior
        });

        carouselInner.addEventListener('pointermove', (event) => {
            if (!isDragging) return; // Only track movement if dragging
            endX = event.clientX || event.touches[0].clientX;
        });

        carouselInner.addEventListener('pointerup', () => {
            if (!isDragging) return; // Only act if dragging occurred
