...
fetch('data.json')
    ...
    .then(data => {
        ...
        let currentIndex = 1; // Start at the first item (the original first item)
        ...
        carousel.addEventListener('touchend', () => {
            if (isSwiping) {
                ...
                if (currentIndex === 0) {
                    ...
                    updateCarousel(); // Update to the last item instantly
                    carousel.style.transition = 'none'; // Disable transition during wrap
                    carousel.style.transform = `translateX(${-currentIndex * 100}%)`; // Set to last item
                } else if (currentIndex === lists.length + 1) {
                    currentIndex = 1; // Go to the first original item
                    updateCarousel(); // Update to the first item instantly
                    carousel.style.transition = 'none'; // Disable transition during wrap
                    carousel.style.transform = `translateX(${-currentIndex * 100}%)`; // Set to first item
                } else {
