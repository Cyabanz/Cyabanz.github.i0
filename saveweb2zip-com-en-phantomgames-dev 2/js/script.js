...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var isExcludedDomain = excludedDomains.some(domain => currentDomain.includes(domain));
    ...
    setDisplay('footer-ad', isExcludedDomain ? 'none' : 'block');
    setDisplay('banner-ad-two', isExcludedDomain ? 'none' : 'block');
    setDisplay('banner-ad-one', isExcludedDomain ? 'none' : 'block');
    setDisplay('game-desc-banner-ad', isExcludedDomain ? 'none' : 'block');
...
if (mobile) {
    ...
    .then(categories => {
        ...
        uniqueGames.forEach(game => {
            ...
            gameElement.classList.add("game");
            gameElement.innerHTML = `
                <img data-src="${game[2]}" class="img" alt="${game[0]}" onclick="play('${game[4] || game[3]}', '${game[3]}', '${game[0]}', '${game[1]}')">
                <h1>${game[0]}</h1>
